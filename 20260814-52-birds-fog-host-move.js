(() => {
  ...
  // Move the territorial model from Breath into the following Host chapter.
  const territorialModel = document.querySelector("#salt-breath .v35-slot-0801");
  const hostList = document.querySelector("#salt-host .salt-media-list") || document.getElementById("salt-host");
  if (territorialModel && hostList) {
    const kicker = territorialModel.querySelector("figcaption span, figcaption .section-kicker");
    if (kicker) kicker.textContent = "CHAPTER .06 / TERRITORIAL MODEL";
    hostList.append(territorialModel);
  }
